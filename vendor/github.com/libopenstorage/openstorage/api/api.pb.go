...
func (m *SdkRole) MarshalToSizedBuffer(dAtA []byte) (int, error) {
   i := len(dAtA)
   _ = i
   var l int
   _ = l
...
func (this *SdkRule) Equal(that interface{}) bool {
    if that == nil {
        return this == nil
    }

    that1, ok := that.(*SdkRule)
    if !ok {
        that2, ok := that.(SdkRule)
        if ok {
            that1 = &that2
        } else {
            return false
...
func (this *SdkRole) Equal(that interface{}) bool {
    if that == nil {
        return this == nil
    }

    that1, ok := that.(*SdkRole)
    if !ok {
        that2, ok := that.(SdkRole)
        if ok {
            that1 = &that2
        } else {
            return false
...
func NewPopulatedSdkRule(r randyApi, easy bool) *SdkRule {
    this := &SdkRule{}
    v60 := r.Intn(10)
...
func NewPopulatedSdkRole(r randyApi, easy bool) *SdkRole {
    this := &SdkRole{}
    this.Name = string(randStringApi(r))
    if r.Intn(5) != 0 {
        v62 := r.Intn(5)
        this.Rules = make([]*SdkRule, v62)
        for i := 0; i < v62; i++ {
            this.Rules[i] = NewPopulatedSdkRule(r, easy)
        }
    }
    if !easy && r.Intn(10) != 0 {
    }
...
func (m *SdkRole) Unmarshal(dAtA []byte) error {
    l := len(dAtA)
    iNdEx := 0
    for iNdEx < l {
        preIndex := iNdEx
        ...
        wireType := int(wire & 0x7)
        if wireType == 4 {
            return fmt.Errorf("proto: SdkRole: wiretype end group for non-group")
        }
        if fieldNum <= 0 {
            return fmt.Errorf("proto: SdkRole: illegal tag %d (wire type %d)", fieldNum, wire)
        }
        switch fieldNum {
        case 1:
            if wireType != 2 {
