...
// CreateMetadata is embedded in both Create and Update requests across all
// APIs. Its fields play different roles depending on which request it is
// embedded in:
//   - On Create: name/org_id/labels/ownership are all supplied by the caller
//     to define the new object. uid is not set by the caller; it is
//     generated by the server.
//   - On Update: name/org_id/uid are read-only identifiers used only to
//     locate the existing object to update; they cannot be changed via
//     Update. labels and ownership are the only fields here that are
//     actually editable via Update.
//
// Update APIs generally do not support partial/delta updates: the caller is
// expected to Inspect/Enumerate the object first, modify only the fields
// that need to change, and send the complete object (including unchanged
// nested fields) back in the Update request. Fields omitted or left at
// their zero value may be reset, unless a message/RPC explicitly documents
// otherwise.
type CreateMetadata struct {
	// name of the object.
	// Update: immutable identifier, used to locate the object being
	// updated. Cannot be changed via Update.
	Name string `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	...
	// uid of the object.
	// Update: immutable identifier, optionally used (together with name) to
	// locate the object being updated. Cannot be changed via Update.
